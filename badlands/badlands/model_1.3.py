...
import io
import os
print ('this is badland test version of lake array')
    ...
    def run_to_time(self, tEnd, verbose=False):
        ...
        while self.tNow < tEnd:
           ...
           # if self.input.tStart != self.tNow:
            ...
            period = 5e6 # yr
            llmax = 200 #100 # m lake change maxmium 
            self.input.fillmax = llmax
            time_max = 5e6 # year
            n_step = 100
            # time
            hist = np.linspace(0, time_max, n_step, dtype = int)
            # lake level changes
            llchange = 0.5*llmax*np.sin(2*np.pi*hist/period- 0.5*np.pi) + 0.5*llmax
            ...
            df['now_ll'] = 0 # make a new column
            df.loc[df['lake_depth']>1, 'now_ll'] = df['lake_depth'] + llchange[ll_id]

            # add on 
            self.fillH = self.elevation + df['now_ll'].values
            # avoid negative fillh
            self.fillH = np.maximum(self.fillH,self.elevation)

            #copy all output from streamflow start with 'co'
            co_fillH = self.fillH
            co_elevation = self.elevation
            co_fv_nei, co_fv_vor, co_fv_len, co_lgids = self.FVmesh.neighbours, self.FVmesh.vor_edges, self.FVmesh.edge_length, self.lGIDs
            co_delta = self.flow.delta
            co_arrdonor = self.flow.arrdonor
            co_maxdonors = self.flow.maxdonors
            co_donors, co_donors1= self.flow.donors,self.flow.donors1
            #co_riverrain = riverrain
            co_base1, co_receivers1 = self.flow.base1, self.flow.receivers1
            co_base, co_receivers = self.flow.base, self.flow.receivers
            co_maxh, co_maxdep = self.flow.maxh, self.flow.maxdep 
            co_localstack, co_localstack1 = self.flow.localstack, self.flow.localstack1
            co_localbase,co_localbase1 = self.flow.localbase, self.flow.localbase1
            co_stack,co_stack1 = self.flow.stack, self.flow.stack1
            co_pitID = self.flow.pitID
            co_pitVolume = self.flow.pitVolume
            co_pitDrain = self.flow.pitDrain
            co_pitallDrain = self.flow.allDrain
            co_contorl_volumes = self.FVmesh.control_volumes
            ...
            #df['sea'] = self.force.sealevel
            #df['cv']= self.FVmesh.control_volumes
            #df.to_csv('testoutput/fristfill_{}.csv'.format(self.tNow))
            df.to_csv('testoutput/fristfill_{}.csv'.format(self.tNow))
            ...
            #self.fillH = elevationTIN.pit_stack(fillH_fix, 1, self.force.sealevel)
            df = pd.DataFrame()
            df['x'] = self.FVmesh.node_coords[:, 0]
            df['y'] = self.FVmesh.node_coords[:, 1]
            df['fillH'] = self.fillH
            ...
            self.flow.compute_parameters_depression(
               ...
               self.force.sealevel)
            # self.force.getRivers(self.tNow)
            # riverrain = self.rain+self.force.rivQw
            # self.flow.compute_flow(self.force.sealevel, self.elevation, 
            #      self.FVmesh.control_volumes, riverrain)
            ...
            #df['sea'] = self.force.sealevel
            #df['cv']= self.FVmesh.control_volumes
            df.to_csv('testoutput/third_fill_{}.csv'.format(self.tNow))

            # paste back original variables except fillH
            # #co_fillH = self.fillH
            self.elevation = co_elevation 
            self.FVmesh.neighbours, self.FVmesh.vor_edges, self.FVmesh.edge_length, self.lGIDs=co_fv_nei, co_fv_vor, co_fv_len, co_lgids 
            self.flow.delta = co_delta 
            self.flow.arrdonor = co_arrdonor 
            self.flow.maxdonors = co_maxdonors 
            self.flow.donors,self.flow.donors1 = co_donors, co_donors1
            #riverrain = co_riverrain 
            self.flow.base1, self.flow.receivers1 = co_base1, co_receivers1
            self.flow.base, self.flow.receivers = co_base, co_receivers   
            self.flow.maxh, self.flow.maxdep = co_maxh, co_maxdep 
            self.flow.localstack, self.flow.localstack1 = co_localstack, co_localstack1 
            self.flow.localbase, self.flow.localbase1 = co_localbase,co_localbase1 
            self.flow.stack, self.flow.stack1 = co_stack,co_stack1  
            self.flow.pitID = co_pitID
            self.flow.pitVolume = co_pitVolume 
            self.flow.pitDrain = co_pitDrain 
            self.flow.allDrain = co_pitallDrain
            self.FVmesh.control_volumes = co_contorl_volumes 
            ...
            #df['sea'] = self.force.sealevel
            #df['cv']= self.FVmesh.control_volumes
            df.to_csv('testoutput/four_fill_{}.csv'.format(self.tNow))
